...
def next_batch_pairwise(data,batch_size,n_negs=1):
    ...
    while ptr < data_size:
        ...
        ratings = [training_data[idx][3] for idx in range(ptr, batch_end)]
        ptr = batch_end
        u_idx, i_idx, j_idx, rating_list = [], [], [], []
        item_list = list(data.item.keys())
        for i, user in enumerate(users):
            i_idx.append(data.item[items[i]])
            u_idx.append(data.user[user])
            rating_list.append(int(ratings[i]))
...
def next_user_samples(data, user_id, n_negs=1):
    user_id = str(user_id)
    if user_id not in data.training_set_u:
        return [], [], []
        # raise ValueError(f"User ID {user_id} not found in training data")

    # 获取用户的正样本
    positive_items = data.training_set_u[user_id]
    ...
    # 构建正样本索引列表
    u_idx = [data.user[user_id]] * len(positive_items)
    i_idx = [data.item[item] for item in positive_items]
...
def next_item_samples(data, item_index, n_negs=1):
    item_index = str(item_index)
    if item_index not in data.training_set_i:
        return [], [], []

    # 获取与该物品交互的正样本用户
    positive_users = data.training_set_i[item_index]

    # 如果没有正样本用户，返回空
    if not positive_users:
        return [], [], []

    item_idx = [data.item[item_index]] * len(positive_users)
    pos_u_idx = [data.user[user] for user in positive_users]

    user_list = list(data.user.keys())
    neg_u_idx = []
    for _ in positive_users:
        for _ in range(n_negs):
            neg_user = random.choice(user_list)
            while neg_user in positive_users:
                neg_user = random.choice(user_list)
            neg_u_idx.append(data.user[neg_user])

    return item_idx, pos_u_idx, neg_u_idx
