...
class LGCN_Encoder(nn.Module):
    def __init__(self, data, emb_size, n_layers, drop_rate, drop_flag=False):
        ...
        self.sparse_norm_adj = TorchGraphInterface.convert_sparse_mat_to_tensor(self.norm_adj).to("cuda:1")
    ...
    def sparse_dropout(self, x, rate, noise_shape):
        ...
        random_tensor += torch.rand(noise_shape).to("cuda:1")
        dropout_mask = torch.floor(random_tensor).type(torch.bool)
        i = x._indices()
        v = x._values()
        i = i[:, dropout_mask]
        v = v[dropout_mask]
        out = torch.sparse.FloatTensor(i, v, x.shape).to("cuda:1")
        return out * (1. / (1 - rate))
